...
run_deadline=min(time.monotonic()+requested*60,read_at+(remaining-30)*60)
completed_before=15000
launch_number=0
next_mode=mode
last_export=173
...
while True:
    ...
    if code!=0:
        detail=stderr.read_text(errors='replace').replace(token,'[REDACTED]')
        print('--- retained stderr tail ---',flush=True)
        print(detail[-6000:],flush=True)
...
print('── BOUNDED RECOVERY ORCHESTRATION STOPPED ──')
