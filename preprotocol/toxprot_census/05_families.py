...
def run():
    ...
    # ── define reference set (non-divergent, non-burned clusters) ────────────
    ref_mask = (div["is_divergent"] == 0) & (div["in_diagnostic_burn"] == 0)
    ref_reps  = set(div[ref_mask]["cluster_rep"])

    # Collect all Pfam families present in the reference set.
    reference_pfam_families = set()
    for rep in ref_reps:
        reference_pfam_families.update(cluster_pfam_families.get(rep, set()))
    ...
    for _, row in div.iterrows():
        ...
        # V2-A: just sequence-divergent
        v2a = is_div and not is_burned
        # V2-B: sequence-divergent AND family-disjoint
        # Missing Pfam: CANNOT be counted as disjoint — flag separately
        if not has_pfam:
            v2b               = False
    ...
    n_no_pfam = int((gv2["annotation_status"] == "missing_pfam").sum())
    n_shared  = int((gv2["annotation_status"] == "shared_pfam").sum())
    n_disjoint = int((gv2["annotation_status"] == "disjoint").sum())
