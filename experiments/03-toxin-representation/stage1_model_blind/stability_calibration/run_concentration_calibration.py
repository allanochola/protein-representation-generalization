...
def concentration_threshold_grid(
    df: pd.DataFrame,
) -> tuple[pd.DataFrame, dict | None]:
    n139 = df[
        df["n_per_class"] == 139
    ...
    for threshold in CONCENTRATION_GRID:
        final_pass = (
            (n139["combined_feature_set_pass"] == 1)
            & (
                n139[
                    "fixed_top5_concentration_median"
                ]
        ...
        for regime in REGIMES:
            ...
            rates[regime] = float(
                final_pass[mask].mean()
            )

        acceptable = (
            rates["C_distributed"] >= 0.70
            and rates["D_diffuse_sixteen"] <= 0.10
            and rates["B_near_tie"] <= 0.10
        )
        ...
        rows.append({
            "concentration_threshold":
                threshold,
            "pass_rate_A_stable_single":
                rates["A_stable_single"],
            "pass_rate_B_near_tie":
                rates["B_near_tie"],
            "pass_rate_C_stable_five":
                rates["C_distributed"],
            "pass_rate_D_diffuse_sixteen":
                rates["D_diffuse_sixteen"],
            "acceptable":
                int(acceptable),
        })

        if acceptable and chosen is None:
            chosen = {
                "concentration_threshold":
                    threshold,
                "pass_rate_A_stable_single":
...
def run():
    ...
    concentration_grid, chosen_concentration = (
        concentration_threshold_grid(
            raw
        )
    )
    ...
    concentration.to_csv(
        HERE / "concentration_summary.csv",
        index=False,
    )

    concentration_grid.to_csv(
        HERE / "concentration_threshold_grid.csv",
        index=False,
    )
    ...
    result = {
        ...
        "concentration_status":
            "CALIBRATED"
            if chosen_concentration is not None
            else "UNCALIBRATED",
    }
