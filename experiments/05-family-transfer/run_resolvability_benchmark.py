...
def main():
    if not BENCHMARK_AUTHORIZED:raise SystemExit('STOP: synthetic benchmark is not authorized')
    ...
    output=exp/'resolvability_benchmark_archive'
    c.require(not output.exists(),'Benchmark output exists; inspect rather than overwrite')
    pin=json.loads((exp/'resolvability_implementation_snapshot.json').read_text())
    ...
    for name,cgroups,egroups,positives,cfirst,efirst in scenarios:
        ...
        for i in range(4):
            ...
            scores=c.generate(d,cell,c.stream('boundary',cell,i,'generation'))
            # Discard all numerical inference results; timings only.
            c.infer(d,scores,c.stream('boundary',cell,i,'bootstrap'),999)
    ...
    with tempfile.TemporaryDirectory(prefix='.resolvability_benchmark_',dir=exp) as tmp:
        ...
        provenance={'runner_sha256':c.digest(Path(__file__).read_bytes()),'implementation_snapshot_sha256':c.digest((exp/'resolvability_implementation_snapshot.json').read_bytes()),
