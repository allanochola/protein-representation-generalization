...
def stream(phase, cell, outer, purpose):
    tokens=[20260924, 'hierarchical_v2', phase, cell['allocation'],
...
def make_design(records):
    ...
    require(all(len(r)==4 and all(isinstance(x,str) and x.isascii() for x in r) for r in records),'Invalid/non-ASCII design record')
...
def prepare_strata(design):
    return ([design['cn'][design['cidx']==g] for g in range(design['C'])],
            [(design['ei'][(design['eidx']==g)&design['ep']],
              design['ei'][(design['eidx']==g)&~design['ep']]) for g in range(design['E'])])

def draw_hierarchy(design,strata,rng):
    calibration,evaluation=strata
    cg=rng.integers(0,design['C'],size=design['C'])
    eg=rng.integers(0,design['E'],size=design['E'])
    cdraw=[];edraw=[]
    for g in cg:
        members=calibration[int(g)]
        cdraw.append(members[rng.integers(0,len(members),size=len(members))])
    for g in eg:
        occurrence=[]
        for members in evaluation[int(g)]:  # positive, then negative
            occurrence.append(members[rng.integers(0,len(members),size=len(members))]
                              if len(members) else members.copy())
        edraw.append(tuple(occurrence))
    return cdraw,edraw

def evaluate_draw(design,scores,draw):
    cdraw,edraw=draw
    require(len(cdraw)==design['C'] and len(edraw)==design['E'],'Wrong occurrence count')
    cn=np.concatenate(cdraw);nc=np.array([len(x) for x in cdraw],dtype=np.int64)
...
def decisions(lower,upper,truth):
    require(math.isfinite(lower) and math.isfinite(upper) and lower<=upper,'Invalid interval')
    return dict(coverage=lower<=truth<=upper,null_rejection=lower>0 or upper<0,
                positive_rejection=lower>0)

def infer(design,scores,rng,bootstrap_replicates=999,batch=32):
    require(scores.shape==(len(design['records']),2) and np.isfinite(scores).all(),'Invalid score shape or values')
    observed=evaluate(design,scores,np.ones((1,design['C']),dtype=np.int64),np.ones((1,design['E']),dtype=np.int64))
    require(isinstance(bootstrap_replicates,int) and bootstrap_replicates>=2,'Invalid bootstrap count')
    require(isinstance(batch,int) and batch>0,'Invalid batch size')
    prepared=prepare_strata(design)
    diffs=[];th=[[],[]]
    # Sequential RNG order is frozen; batch affects neither draws nor arithmetic.
    for start in range(0,bootstrap_replicates,batch):
        for _ in range(start,min(start+batch,bootstrap_replicates)):
            draw=draw_hierarchy(design,prepared,rng)
            b=evaluate_draw(design,scores,draw)
            diffs.append(float(b[1]['tpr'][0]-b[0]['tpr'][0]))
            for a in (0,1):th[a].append(float(b[a]['threshold'][0]))
...
def outer(design,phase,cell,index,bootstrap_replicates=999):
    ...
    try:
        ...
        delta=cell['delta'];result.update(valid=True,outer_index=index,
            **decisions(result['lower'],result['upper'],delta),error=result['estimate']-delta)
        return result
