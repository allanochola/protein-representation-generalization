...
import numpy as np
import resolvability_core as core

EXP=Path(__file__).resolve().parent
REPO=EXP.parents[1]
SPEC=EXP/'resolvability_statistical_spec_001.json'
IMPLEMENTATION=EXP/'resolvability_implementation_snapshot.json'
...
def contract():
    ...
    spec=json.loads(read_committed(SPEC,implementation['statistical_settings_sha256']))
...
def verify_validation_archive(spec,implementation):
    archive=EXP/'resolvability_validation_archive'
    ...
    for item in summary['cells']:
        ...
        require(core.summarize(rows)==item['metrics'],'Validation metrics do not reproduce')
...
def phase_run(phase,runner_file,resume=False):
    ...
    tree=ast.parse(read_committed(EXP/('run_resolvability_'+other+'.py')).decode())
    other_gate=[n.value.value for n in tree.body if isinstance(n,ast.Assign) and isinstance(n.value,ast.Constant)
                and any(isinstance(t,ast.Name) and t.id=='RESOLVABILITY_'+other.upper()+'_AUTHORIZED' for t in n.targets)]
    ...
    checkpoint=Path('/kaggle/working/exp05-resolvability-checkpoints')/(phase+'-'+identity)
    ...
    for cell_index,cell in enumerate(phase_cells):
        ...
        metrics=core.summarize(rows);item={'cell':cell,'metrics':metrics,'replicates_file':f'replicates/cell_{cell_index:03d}.jsonl'}
        if phase=='validation':
            key='null_rejection' if cell['validation']=='V1' else 'coverage'
            limits=spec['validation'][cell['validation']]['type_I_bounds' if key=='null_rejection' else 'coverage_bounds']
