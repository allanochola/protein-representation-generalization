# ===========================================================================
# FINAL RECONCILED DECODER-GEOMETRY AGGREGATION DRIVER
#
# Immediate governing specification:
#   DECODER_GEOMETRY_FINAL_AGGREGATION_SPEC.md
#   SHA-256 54e4110acd3d38148e553d1cdb766fd8130bbb91355edd698678e4c159e69d55
#
# Historical template:
#   decoder_geometry_aggregation.py
#   SHA-256 d71787762271c29add94ccf0907da57bdb59e4c651017301fd638a85dcb23a51
#
# This is a NEW driver. The historical aggregation driver remains unchanged
# and hard-disabled.
# ===========================================================================
...
BOOTSTRAP_STREAMS = {
    ...
    "cmatched_r32_median": 18,
    "cmatched_top32_median": 19,
    "cmatched_neff_median": 20,

    "bio_minus_cmatched_r32": 21,
    "bio_minus_cmatched_top32": 22,
    "bio_minus_cmatched_neff": 23,

    "null_minus_cmatched_r32": 24,
    "null_minus_cmatched_top32": 25,
    "null_minus_cmatched_neff": 26,

    "cmatched_minus_iso_r32": 27,
    "cmatched_minus_iso_top32": 28,
    "cmatched_minus_iso_neff": 29,
}
...
EXPECTED_CMATCHED_POPULATION_MANIFEST_SHA256 = (
    "b839f6f9e0a5d27a06fb32bae55ec8bf5476a43847eac75dcab0e336e6e87ee5"
)

EXPECTED_CMATCHED_STORAGE_MANIFEST_SHA256 = (
    "97330d5689cb57615051b1fd7b4524f9362a8406de4d0aa6c70ccc86bfd46451"
)

EXPECTED_CMATCHED_COMPLETION_NOTE_SHA256 = (
    "c2206e006b56bc775b9ca2e9bdc84963cecbea5863831c7f57598d9aec126863"
)

CMATCHED_POPULATION_NAME = 'cmatched_null'

EXPECTED_CMATCHED_TOTAL = 100
EXPECTED_CMATCHED_ELIGIBLE = 100
EXPECTED_CMATCHED_ZERO = 0

EXPECTED_CMATCHED_ENGINE_SHA256 = (
    "8289d96a11ee3f5506edf298bf0290f58414454737156c98737f4522642915ab"
)

EXPECTED_CMATCHED_RECOVERY_REPORT_SHA256 = (
    "bcc20a34612c9f9c81e546cb6fc74ae6fc2fd81c21b9094ba924a6f36143a45c"
)

EXPECTED_CMATCHED_BETA_ARTIFACT_SHA256 = (
    "a1ac55f9810a0f100e3b9cb4805acbdf5c63a9667980f8229d6478b44be7268c"
)

EXPECTED_CMATCHED_SAE_SHA256 = (
    "bf0dfb992321cf4d1ce80fced0db0256f5c7a1f9fdd8a7fe4834e786c1f6472a"
)

EXPECTED_CMATCHED_RECOVERY_COMPLETION_NOTE_SHA256 = (
...
def verify_cmatched_population_manifest(
    manifest_path: Path,
    records_dir: Path,
) -> List[Dict[str, Any]]:
    ...
    if (
        ...
        raise RuntimeError(
            "C-matched completion manifest SHA mismatch."
        )
    ...
    if (
        manifest.get("status")
        != "complete_unsummarized"
    ):
        raise RuntimeError(
            "C-matched manifest status mismatch."
        )
    ...
    if (
        ...
        raise RuntimeError(
            "C-matched scientific-role mismatch."
        )

    if (
        manifest.get("population")
        != CMATCHED_POPULATION_NAME
    ):
        raise RuntimeError(
            "C-matched population label mismatch."
        )

    expected_scalar_fields = {
        "expected_count": EXPECTED_CMATCHED_TOTAL,
        "accounted_count": EXPECTED_CMATCHED_TOTAL,
        "eligible_nonzero_beta_count":
            EXPECTED_CMATCHED_ELIGIBLE,
        "ineligible_zero_beta_count":
            EXPECTED_CMATCHED_ZERO,
        "geometry_complete_count":
            EXPECTED_CMATCHED_ELIGIBLE,
        "engine_sha256":
            EXPECTED_CMATCHED_ENGINE_SHA256,
    ...
    for key, expected in expected_scalar_fields.items():
        if manifest.get(key) != expected:
            raise RuntimeError(
                "C-matched manifest field mismatch: "
                f"{key}"
            )
    ...
    if not isinstance(
        ...
        raise RuntimeError(
            "C-matched boundary missing."
        )

    expected_false_boundary = (
        "population_summary_computed",
        "biological_geometry_opened_for_comparison",
        "canonical_null_geometry_opened_for_comparison",
        "isotropic_geometry_opened_for_comparison",
        "bootstrap_computed",
        "p_value_computed",
    ...
    for key in expected_false_boundary:
        if boundary.get(key) is not False:
            raise RuntimeError(
                "C-matched pre-aggregation boundary mismatch: "
                f"{key}"
            )
    ...
    if (
        ...
        or len(entries)
        != EXPECTED_CMATCHED_TOTAL
    ):
        raise RuntimeError(
            "C-matched manifest record count mismatch."
        )

    expected_null_ids = tuple(
        range(
            1100001,
            1100101,
        )
    ...
    for entry in entries:
        ...
        if not isinstance(
            ...
            raise RuntimeError(
                "Malformed C-matched manifest entry."
            )
        ...
        if (
            ...
            raise RuntimeError(
                "Malformed C-matched path/hash entry."
            )
        ...
        if not record_path.is_file():
            raise RuntimeError(
                "Missing C-matched geometry record: "
                f"{record_path.name}"
            )
        ...
        if null_id in by_null_id:
            raise RuntimeError(
                "Duplicate C-matched null ID."
            )
        ...
        if (
            ...
            raise RuntimeError(
                "C-matched record population mismatch."
            )

        if (
            entry.get("perturbation_id")
            != null_id
        ):
            raise RuntimeError(
                "C-matched manifest/record null-ID mismatch."
            )

        if (
            entry.get("biological_source_id")
            != bio_id
        ):
            raise RuntimeError(
                "C-matched manifest/record source-ID mismatch."
            )
        ...
        if (
            ...
            raise RuntimeError(
                "C-matched eligibility mismatch."
            )
        ...
        if (
            ...
            raise RuntimeError(
                "C-matched analysis-status mismatch."
            )
    ...
    if observed_ids != expected_null_ids:
        raise RuntimeError(
            "C-matched null-ID universe mismatch."
        )
    ...
    ordered = [
        ...
        for null_id in expected_null_ids
    ]

    observed_bio_ids = tuple(
        int(
            row[
                "biological_source_id"
            ]
    ...
    if observed_bio_ids != expected_bio_ids:
        raise RuntimeError(
            "C-matched ordinal biological-source mapping mismatch."
        )
...
def headline_analysis(
    biological_geometry: Sequence[Dict[str, Any]],
    null_geometry: Sequence[Dict[str, Any]],
    isotropic_geometry: Sequence[Dict[str, Any]],
    cmatched_geometry: Sequence[Dict[str, Any]],
) -> Dict[str, Any]:

    arrays = {}

    for population, geometries in (
        ("bio", biological_geometry),
        ("null", null_geometry),
        ("iso", isotropic_geometry),
        ("cmatched", cmatched_geometry),
    ):
    ...
    single_streams = {
        ...
        ("cmatched", "r32"):
            "cmatched_r32_median",
        ("cmatched", "top32"):
            "cmatched_top32_median",
        ("cmatched", "neff"):
            "cmatched_neff_median",
    }
...
def run_aggregation(
    ...
    cmatched_manifest_path: Path,
    cmatched_records_dir: Path,
    ...
    for path in (
        ...
        cmatched_manifest_path,
        cmatched_records_dir,
        output_path,
    ):
        assert_no_confirmatory_path(
            path
    ...
    if (
        ...
        raise RuntimeError(
            "Final aggregation specification SHA mismatch."
        )
    ...
    if output_path.exists():
        raise RuntimeError(
            "Refusing to overwrite existing final aggregation output."
        )
    ...
    cmatched = (
        verify_cmatched_population_manifest(
            cmatched_manifest_path,
            cmatched_records_dir,
        )
    )

    cmatched_eligible, cmatched_zero = (
        split_population(
            cmatched,
            EXPECTED_CMATCHED_ELIGIBLE,
            EXPECTED_CMATCHED_ZERO,
    ...
    # Structural accounting must close before scientific summaries.
    if (
        len(biological_eligible) != 100
        or len(biological_zero) != 0
        or len(null_eligible) != 63
        or len(null_zero) != 37
        or len(cmatched_eligible) != 100
        or len(cmatched_zero) != 0
        or len(isotropic) != 1000
    ):
        raise RuntimeError(
            "Final structural population accounting mismatch."
        )

    # The result object contains all populations but does not assign
    # significance thresholds or PASS/FAIL status.
    result = {
        "state":
            "DECODER_GEOMETRY_FINAL_AGGREGATION_COMPLETE",
        "scientific_status":
            "post_hoc_descriptive",
        "governing_spec_sha256":
            EXPECTED_AGGREGATION_SPEC_SHA256,
        "primary_comparison":
            "biological_vs_canonical_permutation_null",
        "secondary_comparisons": [
            "biological_vs_isotropic",
            "biological_vs_cmatched_permutation_null",
        ],
        ...
        "population_accounting": {
            ...
            "cmatched_null_total": 100,
            "cmatched_null_geometry_eligible": 100,
            "cmatched_null_zero_beta": 0,
        ...
        "storage_deviation": {
            "isotropic_overflow":
                "indices_951_999_stored_in_private_overflow_dataset",
            "scientific_effect":
                "storage_only",
        },
    }
    ...
    print(
        "PASS — final decoder-geometry aggregation completed."
    )
    print(
        "PASS — biological: 100 eligible, 0 zero."
    )
    print(
        "PASS — canonical null: 63 eligible, 37 zero."
    )
    print(
        "PASS — C-matched null: 100 eligible, 0 zero."
    )
    print(
        "PASS — isotropic: 1000 directions."
    )
    print(
    ...
    print(
        "STATE = DECODER_GEOMETRY_FINAL_AGGREGATION_COMPLETE"
    )
...
def main() -> int:
    ...
    if not ENABLE_AGGREGATION:
        ...
        print(
            "EXP 04 FINAL DECODER GEOMETRY AGGREGATION: HARD-DISABLED"
        )

        print(
            "Final superseding aggregation specification is frozen."
        )
        ...
        print(
            "No C-matched geometry payload opened."
        )
    ...
    run_aggregation(
        aggregation_spec_path=
            args.aggregation_spec,
        population_manifest_path=
            args.population_manifest,
        population_records_dir=
            args.population_records_dir,
        cmatched_manifest_path=
            args.cmatched_manifest,
        cmatched_records_dir=
            args.cmatched_records_dir,
        isotropic_roots=
            args.isotropic_root,
        output_path=
            args.output,
