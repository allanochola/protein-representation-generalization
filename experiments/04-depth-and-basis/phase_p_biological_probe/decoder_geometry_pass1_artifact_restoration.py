...
# ---------------------------------------------------------------------------
# PASS-1 ARTIFACT RESTORATION AUTHORITY
# ---------------------------------------------------------------------------

PASS1_ARTIFACT_RESTORATION_SPEC_FILENAME = (
    "DECODER_GEOMETRY_PASS1_ARTIFACT_RESTORATION_SPEC.md"
)

EXPECTED_PASS1_ARTIFACT_RESTORATION_SPEC_SHA256 = (
    "5a348031eb510de736a1e22db4d3f03478fc3ee3c81793c93b25e7064be28bd2"
)

EXPECTED_HISTORICAL_PASS1_AUTHORIZED_SOURCE_SHA256 = (
...
def verify_pass1_artifact_restoration_authority() -> None:
    """Verify frozen public restoration authority before protected replay."""
    restoration_spec_path = (
        Path(__file__).resolve().parent
        / PASS1_ARTIFACT_RESTORATION_SPEC_FILENAME
    )

    if not restoration_spec_path.is_file():
        raise RuntimeError(
            "Pass-1 artifact restoration specification is missing."
        )
    ...
    if (
        ...
        raise RuntimeError(
            "Pass-1 artifact restoration specification SHA mismatch: "
            f"{observed_sha}"
        )


def main() -> int:
    verify_pass1_artifact_restoration_authority()
    verify_static_identities()
